...
def init_game():
    ...
    game_state = {
        "particles": [],
        "immobiles": [
        # {"pos": (320, 250)}, {"pos": (320, 150)}
        ],
        "particle_ranges": [],
        "controls": {"dir_1": 0},
        # rect x1,y1,x2,y2
        "targets": [
        # {"rect": (150, 0, 190, 50), "img": target_img, "temp": min_temp, "factor": 1}, # 190
    ...
    for i in range(range_start, range_stop + 1):
        ...
        if not particle["enabled"] and game_state["reservoir"] > game_state["max_flow_rate"]:
            if random.random() < game_state["flow_rate"]:
                ...
                particle["velocity"].update(random.uniform(-0.5, 0.5), 20 + random.uniform(-0.5, 0.5))
                particle["velocity"].rotate_ip(-game_state["angle"])
...
def update_game(surface, game_state, frame):
    ...
    if controls["dir_1"] != 0:
        game_state["angle"] = max(-game_state["angle_limit"], min(game_state["angle_limit"], game_state["angle"] + game_state["angle_delta"] * controls["dir_1"]))

    for i in range(len(game_state["particle_ranges"])):
        update_flow(game_state, i)
